...
#     def fit(self, X, y=None, additional_features=None):
#         """
...
#         # Store additional features if provided
#         self.has_additional_features_ = additional_features is not None
#         self.original_additional_features_ = additional_features
...
#             # Initialize additional features result
#             result_additional_features = {}
#             if self.has_additional_features_:
#                 for feature_name, feature_values in self.original_additional_features_.items():
#                     result_additional_features[feature_name] = feature_values.copy()
...
#                     # Handle additional features for synthetic samples
#                     if self.has_additional_features_:
#                         for feature_name, feature_values in self.original_additional_features_.items():
#                             # Select random feature values from the same sentiment class
#                             synthetic_feature_values = self._sample_feature_values(
#                                 feature_values=feature_values,
#                                 indices=sentiment_indices,
#                                 num_samples=len(synthetic_samples)
#                             )
#                             result_additional_features[feature_name].extend(synthetic_feature_values)
...
#             if self.has_additional_features_:
#                 for feature_name, feature_values in result_additional_features.items():
...
#             # Store the balanced additional features
#             if self.has_additional_features_:
#                 self.balanced_additional_features_ = {}
#                 for feature_name in result_additional_features.keys():
...
#         This can be useful for other components in the pipeline.
...
#     def _sample_feature_values(self, feature_values, indices, num_samples):
#         """
#         Sample feature values from the specified indices for synthetic samples.
#         """
#         values_to_sample_from = [feature_values[i] for i in indices]
#         sampled_values = random.choices(values_to_sample_from, k=num_samples)
#         return sampled_values
    
#     def get_balanced_features(self, feature_name=None):
#         """
#         Get the balanced additional features after transformation.
#         """
#         if not hasattr(self, 'balanced_additional_features_'):
#             logger.warning("balanced_additional_features_ not available, returning original features")
#             if feature_name:
#                 return self.original_additional_features_.get(feature_name, None)
#             return self.original_additional_features_
        
#         if feature_name:
#             return self.balanced_additional_features_.get(feature_name, None)
...
class SentimentBalancer(PreprocessorBase):
    ...
    def get_balanced_additional_features(self):
        ...
        --------
        dict or None
            Dictionary with feature names as keys and balanced feature values as values,
            or None if no additional features were provided.
        """
